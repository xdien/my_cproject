...
static void mainwin_set_icon (GtkWidget *win)
{
	...
	icon_atom = (Atom) gdk_atom_intern("KWM_WIN_ICON", FALSE);
...
void handle_cmd_line_options(struct cmdlineopt *opt, gboolean remote)
{
	...
	if (opt->toggle_shuffle)
	{
		if (opt->shuffle)
		{
			if (g_ascii_strncasecmp(opt->shuffle, "on", 1) == 0)
			{
				if(!xmms_remote_is_shuffle(opt->session))
		 			xmms_remote_toggle_shuffle(opt->session);
			}
			else if (g_ascii_strncasecmp(opt->shuffle, "off", 2) == 0)
			{
				if(xmms_remote_is_shuffle(opt->session))
		 			xmms_remote_toggle_shuffle(opt->session);
			}
	...
	if (opt->toggle_repeat)
	{
		if (opt->repeat)
		{
			if (g_ascii_strncasecmp(opt->repeat, "on", 2) == 0)
			{
				if(!xmms_remote_is_repeat(opt->session))
		 			xmms_remote_toggle_repeat(opt->session);
			}
			else if (g_ascii_strncasecmp(opt->repeat, "off", 3) == 0)
			{
				if(xmms_remote_is_repeat(opt->session))
		 			xmms_remote_toggle_repeat(opt->session);
			}
	...
	if (opt->toggle_advance)
	{
		if (opt->advance)
		{
			if (g_ascii_strncasecmp(opt->advance, "on", 1) == 0)
			{
				if(!xmms_remote_is_advance(opt->session))
		 			xmms_remote_toggle_advance(opt->session);
			}
			else if (g_ascii_strncasecmp(opt->advance, "off", 2) == 0)
			{
				if(xmms_remote_is_advance(opt->session))
		 			xmms_remote_toggle_advance(opt->session);
			}
...
int main(int argc, char **argv)
{
	...
	//g_thread_init(NULL); 	The GLib threading system is automatically initialized at the start of your program.
	gtk_set_locale();
	...
	/*---------------------------------------------------*/
	sm_client_id = sm_init(argc, argv, options.previous_session_id);/*da test*/
	gdk_set_sm_client_id(sm_client_id);/*da test*/
	mainwin_create();/*da test*/

	filename = g_strconcat(g_get_home_dir(), "/.xmms/gtkrc", NULL);/*da test*/
	gtk_init(NULL,NULL);
	...
	GDK_THREADS_ENTER();
	mainwin_set_info_text(); /*da test*/

	gtk_widget_show(mainwin);/*da test*/
	if (pposition_broken && cfg.player_x != -1 && cfg.save_window_position)
		dock_set_uposition(mainwin, cfg.player_x, cfg.player_y);
	...
	draw_main_window(TRUE);/*da test*/
	
	mainwin_timeout_tag = gtk_timeout_add(10, idle_func, NULL);/*da test*/
	playlist_start_get_info_thread();/*da test*/

	enable_x11r5_session_management(argc, argv);/*da test*/
	gtk_main();
	GDK_THREADS_LEAVE();
	return 0;
}
