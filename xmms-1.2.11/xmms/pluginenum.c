...
void init_plugins(void)
{
	...
	while (node)
	{
		 ...
		 */
		if (!strcmp(g_path_get_basename(cfg.outputplugin),
...
static int plugin_check_duplicate(char *filename)
{
	...
	for (l = ip_data->input_list; l; l = l->next)
		if (!strcmp(base_filename,
			    g_path_get_basename(((InputPlugin*)l->data)->filename)))
			return 1;

	for (l = op_data->output_list; l; l = l->next)
		if (!strcmp(base_filename,
			    g_path_get_basename(((OutputPlugin*)l->data)->filename)))
			return 1;

	for (l = ep_data->effect_list; l; l = l->next)
		if (!strcmp(base_filename,
			    g_path_get_basename(((EffectPlugin*)l->data)->filename)))
			return 1;

	for (l = gp_data->general_list; l; l = l->next)
		if (!strcmp(base_filename,
			    g_path_get_basename(((GeneralPlugin*)l->data)->filename)))
			return 1;

	for (l = vp_data->vis_list; l; l = l->next)
		if (!strcmp(base_filename,
			    g_path_get_basename(((VisPlugin*)l->data)->filename)))
			return 1;
